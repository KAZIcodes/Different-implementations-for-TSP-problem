...
public class HeldKarp {
    ...
    private static final PairDouble maxPair = new PairDouble(MAX_DOUBLE, MAX_DOUBLE);
    private double opt;
    private int dictionaryEntries;

    public HeldKarp(double[][] matrix, int startingCity) {
        setMatrix(matrix);
    ...
    public List<Integer> calculateHeldKarp() {
        ...
        Map<PairDouble, PairDouble> dict = new HashMap<>(getEntriesNum(size - 1) + 1, (float)1.0);

        // Set distances between main cities (except initial)
        for(int i = 1; i < size; i++) {
            dict.put(new PairDouble(1 << i, i),
        ...
        for(int subsetSize = 2; subsetSize < size; subsetSize++) {
            ...
            while(combs.numLeft > 0) {
                ...
                for(int elementj : a) {
                    ...
                    for(int elementk : a) {
                        if(elementk != elementj) {
                            resultTemp.add(new PairDouble(
                                    dict.get(new PairDouble(prev, elementk)).first + matrix[elementk][elementj],
                                    elementk));
                        }
                    }
                    ...
                    dict.put(new PairDouble(bits, elementj), min);
                }
            }
        }
        ...
        for(int i = 1; i < size; i++) {
            resultTemp.add(new PairDouble(
                    dict.get(new PairDouble(bits, i)).first + matrix[i][0],
                    i));
        }
        ...
        min = maxPair;
        for(PairDouble element : resultTemp) {
            if(element.first < min.first || (element.first == min.first && element.second < min.second)) {
                min = element;
            }
        }
        ...
        for(int i = 0; i < size - 1; i++) {
            ...
            parent = (int) dict.get(new PairDouble(bits, parent)).second;
            bits = newBits;
        }
        //System.out.println("\n\n" + path + "\n\n");
    ...
    public void setStartingCity(int startingCity) {
        this.startingCity = TSPDynamicProgramming.constrain(0, this.matrix.length - 1, startingCity);
    }
